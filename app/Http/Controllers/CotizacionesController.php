...
class CotizacionesController extends Controller
{
    ...
    public function findExecute(Request $request) //esta roto revisar
    {
        ...
        $bancos = Bancos::get();
        $proveedores = Proveedor::catalogoPrincipal()->get();
        $empresas = Empresas::get();
...
foreach ($cotizacionesCreadas as $cotizacion) {
 ...
 $this->CostosService->syncDesdeRequest($viaje, $request);
    ...
    public function store(Request $request)
    {
        ...
        $cliente = $request->get('id_cliente');
        $origen_inicial = $request->get('origen_captura');
        ...
        if (!$user->can('mec-ocultar_datos_facturacion')) {
...
->when($idEmpresa != 0, function ($query) use ($idEmpresa) {
    $query->where('ce.id_empresa', $idEmpresa);
     ...
     //   where('id_empresa', $idEmpresa)
        ...
        //nuevo refactor costos cotizacion

        $mapaCostos = config('CatAuxiliares.costosViajes');

        $viaje = $cotizacion->viajes()
    ->where('estado', 'activo')
    ->with('costos')
    ->first();
        ...
        foreach ($mapaCostos as $input => $config) {
            ...
            $costo = $viaje?->costos->firstWhere('concepto', $concepto);

            $costosForm[$input] = $costo?->monto ?? 0;
        }

        $sobrepeso = $viaje?->costos->firstWhere('concepto', 'sobrepeso');
...
$this->procesarDocumento(
    ...
    'CCP'
            ...
            if ($request->has('id_proveedor')) {
                $idProveedorReq = (int) $request->id_proveedor;
                if ($idProveedorReq !== (int) $cotizaciones->id_empresa) { //checar si se cambio de empresa
                    $cotizaciones->id_empresa = $idProveedorReq;
                    $doc_cotizaciones->id_empresa = $idProveedorReq;
                    $doc_cotizaciones->update();
                }
            }
            ...
            if ($request->has('id_transportista')) {
                ...
                if ($idTransportistai !== (int) $cotizaciones->id_proveedor) { //checar si se cambio de proveedor
            ...
            $user = Auth::User();
            // Validar si el usuario tiene permiso para capturar/generar Carta Porte (Cliente 2)
            if (!$user->can('mec-ocultar_datos_facturacion')) {
                $modifico = (int) $request->get('modifico_informacion', 0);

                if ($request->filled('id_uso_cfdi')) $cotizaciones->sat_uso_cfdi_id = $request->id_uso_cfdi;
                if ($request->filled('id_forma_pago')) $cotizaciones->sat_forma_pago_id = $request->id_forma_pago;
                if ($request->filled('id_metodo_pago')) $cotizaciones->sat_metodo_pago_id = $request->id_metodo_pago;

                if ($request->has('direccion_recinto')) $cotizaciones->direccion_recinto = $request->direccion_recinto;
                if ($request->has('text_recinto')) $cotizaciones->uso_recinto = ($request->text_recinto == 'recinto-si') ? 1 : 0;

                // Actualizar campos de Carta Porte
                if ($request->has('cp_fraccion')) $cotizaciones->cp_fraccion = $request->cp_fraccion;
                if ($request->has('cp_clave_sat')) $cotizaciones->cp_clave_sat = $request->cp_clave_sat;
                if ($request->has('cp_pedimento')) $cotizaciones->cp_pedimento = $request->cp_pedimento;
                if ($request->has('cp_clase_pedimento')) $cotizaciones->cp_clase_ped = $request->cp_clase_pedimento;
                if ($request->has('cp_cantidad')) $cotizaciones->cp_cantidad = $request->cp_cantidad;
                if ($request->has('cp_valor')) $cotizaciones->cp_valor = $request->cp_valor;
                if ($request->has('cp_moneda_valor')) $cotizaciones->cp_moneda = $request->cp_moneda_valor;
                if ($request->has('cp_contacto_entrega')) $cotizaciones->cp_contacto_entrega = $request->cp_contacto_entrega;
                if ($request->has('cp_fecha_tentativa_entrega')) $cotizaciones->cp_fecha_tentativa_entrega = $request->cp_fecha_tentativa_entrega;
                if ($request->has('cp_hora_tentativa_entrega')) $cotizaciones->cp_hora_tentativa_entrega = $request->cp_hora_tentativa_entrega;
                if ($request->has('cp_comentarios')) $cotizaciones->cp_comentarios = $request->cp_comentarios;

                // Definir ruta del PDF dentro de public
                $path = public_path('cotizaciones/cotizacion' . $cotizaciones->id . '/formato_carta_porte_' . $numContenedor . '.pdf');
                $folderPath = public_path('cotizaciones/cotizacion' . $cotizaciones->id);
                $pdfExiste = File::exists($path);

                // Detección robusta de si debe generarse o regenerarse el documento:
                // 1) Si no existe físicamente en el disco
                // 2) Si se reportó modifico_informacion == 1 desde el formulario
                // 3) Si cambió el contenedor respecto al original
                // 4) Si hubo cambios en cualquier dato relevante de Carta Porte o generales visibles en el documento
                // 5) Si el request contiene datos específicos de carta porte
                $debeGenerarPdf = !$pdfExiste
                    || $modifico === 1
                    || ($contenedorSave && $contenedorSave->num_contenedor !== $numContenedor)
                    || $cotizaciones->isDirty([
                        'cp_fraccion', 'cp_clave_sat', 'cp_pedimento', 'cp_clase_ped',
                        'cp_cantidad', 'cp_valor', 'cp_moneda', 'cp_contacto_entrega',
                        'cp_fecha_tentativa_entrega', 'cp_hora_tentativa_entrega', 'cp_comentarios',
                        'direccion_entrega', 'origen', 'id_subcliente', 'latitud', 'longitud',
                        'sat_uso_cfdi_id', 'sat_forma_pago_id', 'sat_metodo_pago_id',
                        'direccion_recinto', 'uso_recinto'
                    ])
                    || $request->filled('cp_pedimento')
                    || $request->filled('cp_fraccion');

                if ($debeGenerarPdf) {
                    Log::channel('daily')->info('Regenerando/generando formato carta porte para cotizacion ID: ' . $id . ' Contenedor: ' . $numContenedor);
                    ...
                    if (!File::exists($folderPath)) {
                        ...
                        // Si el archivo anterior existe, lo eliminamos para guardar la versión fresca
                        if (File::exists($path)) {
                            Log::channel('daily')->info('borrando archivo carta porte previo: ' . $path);
                            File::delete($path);
                        }
                    }
    ...
    public function update(Request $request, $id) //refactor para full
    {
        ...
        DB::beginTransaction();
        // crear priemro el viaje que sera nuestra representacion

        $viaje = $this->resolverViajeDesdeRequest($request, $id);
...
*/
            ...
            $viaje->cotizaciones()->syncWithoutDetaching([$cotizaciones->id]);
    ...
    private function resolverViajeDesdeRequest($request, $id)
    {
        if ($request->TipoCotizacion == "Full") {
        ...
        try {
            ...
            if ($pagoInmediato) {
                ...
                $data = [
                    ...
                    'concepto' => 'Pago gasto : '.$numContenedor .' - '. $gasto->tipo,
    ...
    [
        ...
        'abono' => number_format($g['Monto'], 2, '.', '')
                ...
                $data = [
                    ...
                    'tipo' => 'cargo',
                    'monto' => floatval($gasto->cantidad),
                    'concepto' => 'Pago gasto: ' . $r->numContenedor . ' - ' . $gasto->tipo,
...
$urlDocumento = asset("cotizaciones/cotizacion{$id_cot}/{$nameArchivo}");
    ...
    public function storelocal(Request $request)
    {
        ...
        try {
            ...
            $origen_inicial = $request->origen_captura;
            $nuevoSelloBool = null;
            $confirmacionSelloBool = false;
            if ($request->has('nuevo_sello')) {
                $nuevoSelloBool = $request->boolean('nuevo_sello');
            } else {
                $nuevoSelloBool = null;
            }

            if ($request->has('confirmacion_sello')) {
                $confirmacionSelloBool = $request->boolean('confirmacion_sello');
            } else {
            ...
            $cotizacion = Cotizaciones::create([
                ...
                'costo_maniobra_local' => (float) $request->input('Costomaniobra', 0.00),
                ...
                'transportista_local' => $idProveedorFinal,
                'en_patio'            => 1,
                ...
                'tarifa_estadia'       => (float) $request->input('tarifa_estadia', 0.00),
                'total_estadia'        => (float) $request->input('total_estadia', 0.00),
                'dias_pernocta'        => (int) $request->input('dias_pernocta', 0),
                'tarifa_pernocta'      => (float) $request->input('tarifa_pernocta', 0.00),
                'total_pernocta'       => (float) $request->input('total_pernocta', 0.00),
                'total_general'        => (float) $request->input('total_general', 0.00),
...
$this->procesarDocumento(
   ...
   $cot,
    ...
    'BoletaPatio'
            ...
            $cot->origen_local        = $request->origen ?? null;
            $cot->destino_local        = $request->destino ?? null;
            $cot->costo_maniobra_local        = $request->Costomaniobra ?? null;
            $cot->estado_contenedor        = $request->estado_contenedor ?? null;
            ...
            $cot->  tarifa_estadia     = (float) $request->input('tarifa_estadia', 0.00);
            $cot-> total_estadia       = (float) $request->input('total_estadia', 0.00);
            $cot-> dias_pernocta       = (int) $request->input('dias_pernocta', 0);
            $cot-> tarifa_pernocta     = (float) $request->input('tarifa_pernocta', 0.00);
            $cot->total_pernocta       = (float) $request->input('total_pernocta', 0.00);
            $cot-> total_general       = (float) $request->input('total_general', 0.00);
        ...
        try {
            ...
            foreach ($contenedores as $contenedor) {
                ...
                if ($request->has('permiso_proveedor') && $request->get('permiso_proveedor') == 1) {
                    ...
                    $cotizaciones->id_empresa = $numProveedor;
                    $cotizaciones->id_proveedor = $numTransportista;
                    $cotizaciones->empresa_local        = $request->id_proveedor ?? null;
                }
                if ($request->has('id_transportista') && $request->id_transportista !== $cotizaciones->id_proveedor) {
                    $cotizaciones->id_proveedor      = $request->id_transportista;
                    $cotizaciones->transportista_local        = $request->id_transportista ?? null;
                }
                ...
                $cotizaciones->peso_contenedor = $contenedor[5 + $sumarIndex] ?? 0;

                $cotizaciones->bloque = $contenedor[14 + $sumarIndex] ?? null;
                $cotizaciones->bloque_hora_i = $contenedor[15 + $sumarIndex] ?? null;
                $cotizaciones->bloque_hora_f = $contenedor[16 + $sumarIndex] ?? null;
                $cotizaciones->direccion_entrega = $contenedor[17 + $sumarIndex] ?? null;
                ...
                //local
                $cotizaciones->bloque_local               = $contenedor[14 + $sumarIndex] ?? null;
                $cotizaciones->bloque_hora_i_local        = $contenedor[15 + $sumarIndex] ?? null;
                $cotizaciones->bloque_hora_f_local        = $contenedor[16 + $sumarIndex] ?? null;


                $cotizaciones->origen_local        = $request->origen ?? null;
                $cotizaciones->destino_local        = $request->destino ?? null;
                $cotizaciones->costo_maniobra_local        = 0;
                $cotizaciones->estado_contenedor        = 'AMARILLO';
                $cotizaciones->fecha_modulacion_local        =  $contenedor[12 + $sumarIndex] ?? null;

                $cotizaciones->puerto               = 'Lazaro';
                $cotizaciones->fecha_ingreso_puerto =  null;
                $cotizaciones->fecha_salida_puerto  = null;
                $cotizaciones->dias_estadia         = 0;
                $cotizaciones->tarifa_estadia       =  0;
                $cotizaciones->total_estadia        =  0;
                $cotizaciones->dias_pernocta        = 0;
                $cotizaciones->tarifa_pernocta      =  0;
                $cotizaciones->total_pernocta       = 0;
                $cotizaciones->total_general        = 0;
                $cotizaciones->motivo_demora        = null;
                $cotizaciones->liberado             = 0;
                $cotizaciones->fecha_liberacion     = null;
                $cotizaciones->responsable          = 'Viaje local'; //solo para saber de donde nace el viaje TEMPORAL
                $cotizaciones->observaciones        = 'Contenedor cargado desde carga masiva local';
