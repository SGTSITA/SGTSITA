...
class LimpiarHistoriales extends Command
{
     ...
     * @var string
     */
    protected $signature = 'db:limpiar-historiales {--dry-run : Muestra los registros que se borrarían sin borrarlos} {--force : Forzar la depuración e ignorar frecuencia de 30/60 días}';
     ...
     * @var string
     */
    protected $description = 'Respalda en archivos ZIP y depura coordenadas (cada 30 días) e historiales (cada 60 días).';
    ...
    public function handle()
    {
        ...
        // Comprobar cuándo fue el último respaldo de cada tabla
        $ultimoRespaldoCoords = $this->getUltimoRespaldoFecha('coordenadas_historial');
        $diasCoords = $ultimoRespaldoCoords ? (int) $ultimoRespaldoCoords->diffInDays(Carbon::now()) : null;

        $ultimoRespaldoAct = $this->getUltimoRespaldoFecha('activity_log');
        $diasAct = $ultimoRespaldoAct ? (int) $ultimoRespaldoAct->diffInDays(Carbon::now()) : null;

        // Fechas límite de registros a depurar
        $fechaCoordenadas = Carbon::now()->subDays(30);
        $fechaActivity = Carbon::now()->subDays(60);

        // 1. Procesar Coordenadas (frecuencia cada 30 días)
        $this->info("--------------------------------------------------");
        if (!$force && $ultimoRespaldoCoords !== null && $diasCoords < 30) {
            $this->comment("OMITIDO: Coordenadas historial. El último respaldo fue hace {$diasCoords} días (frecuencia requerida: cada 30 días).");
        } else {
            $this->info("Procesando coordenadas_historial (registros anteriores a " . $fechaCoordenadas->toDateString() . ")...");
        ...
        // 2. Procesar Activity Logs (frecuencia cada 60 días)
        $this->info("--------------------------------------------------");
        if (!$force && $ultimoRespaldoAct !== null && $diasAct < 60) {
            $this->comment("OMITIDO: Activity log. El último respaldo fue hace {$diasAct} días (frecuencia requerida: cada 60 días).");
        } else {
            $this->info("Procesando activity_log (registros anteriores a " . $fechaActivity->toDateString() . ")...");
    ...
    /**
     * Limpia los archivos de respaldo ZIP antiguos (más de 180 días) en el servidor.
     */
    private function cleanOldBackups()
    {
        $this->info("Limpiando respaldos ZIP antiguos del servidor (más de 180 días)...");
        $directory = 'historial_backups';

        if (Storage::exists($directory)) {
            $files = Storage::files($directory);
            $limite = Carbon::now()->subDays(180)->getTimestamp();
            $deletedCount = 0;
    ...
    /**
     * Obtiene la fecha del último archivo de respaldo ZIP generado para un tipo específico.
     *
     * @param string $prefix
     * @return Carbon|null
     */
    private function getUltimoRespaldoFecha($prefix)
    {
        $directory = 'historial_backups';
        if (!Storage::exists($directory)) {
            return null;
        ...
        foreach ($files as $file) {
            ...
            if (strpos($filename, $prefix) === 0 && pathinfo($file, PATHINFO_EXTENSION) === 'zip') {
                $lastModified = Storage::lastModified($file);
                if ($latestTimestamp === null || $lastModified > $latestTimestamp) {
