...
function actualizarUbicacion(
    ...
    dataUbi.forEach((item, index) => {
        ...
        if (markers[markerKey]) {
            ...
            // Update map visibility status for existing markers
            markers[markerKey].setMap(
                markerDebeMostrarse({
...
">
    <div style="color:white; font-size:13px; line-height:1.5;">
        ...
        <div><strong>Placas:</strong> ${filtroEqu?.placas || "sin placas"}</div>
                ...
                const hiddenText = item.chasisCercanosOcultos
                    ? `
                    <div style="background-color:rgba(255,193,7,0.85); color:#212529; font-size:11px; padding:6px; border-radius:4px; margin-bottom:4px; font-weight:bold; text-align:center;">
                        ⚠️ Chasis agrupados por cercanía: ${item.chasisCercanosOcultos}
                    </div>
                `
                    : "";
...
function limpiarMapa() {
    ...
    document.getElementById("vistaCheckTodos").checked = false;
}
...
function buscarUbicaciones() {
    ...
    return fetch("/coordenadas/ubicacion-vehiculo", {
        ...
        .then((envelope) => {
            ...
            const isDesactivado = $("#desactivarAgrupamientoInteligente").is(
                ":checked",
            );
            const alertaActiva = isDesactivado
                ? false
                : envelope.alerta_distancia || false;
            const metrosAlerta = envelope.metros_alerta || 50;

            // Apply smart distance filter: check proximity between camion and chasis markers for each container
            let filteredList = [...listData];
            const idsToHide = new Set();
            const closeDistancesMap = {};

            if (alertaActiva) {
                // Group points by container ID
                const grouped = {};
                listData.forEach((item) => {
                    if (item.id_contenendor) {
                        if (!grouped[item.id_contenendor]) {
                            grouped[item.id_contenendor] = [];
                        }
                        grouped[item.id_contenendor].push(item);
                    }
                });
                ...
                const userQuieroVerSoloChasis =
                    !filtrosVistaMapa.has("todos") &&
                    (filtrosVistaMapa.has("ChasisA") ||
                        filtrosVistaMapa.has("ChasisB"));

                if (!userQuieroVerSoloChasis) {
                    Object.keys(grouped).forEach((contId) => {
                        const points = grouped[contId];
                        const tracto = points.find(
                            (p) => p.TipoEquipo === "Camion",
                        );
                        const chasisA = points.find(
                            (p) => p.TipoEquipo === "ChasisA",
                        );
                        const chasisB = points.find(
                            (p) => p.TipoEquipo === "ChasisB",
                        );

                        const hasTractoGPS =
                            tracto &&
                            tracto.ubicacion &&
                            parseFloat(tracto.ubicacion.lat) !== 0 &&
                            parseFloat(tracto.ubicacion.lng) !== 0;

                        if (hasTractoGPS) {
                            const latT = parseFloat(tracto.ubicacion.lat);
                            const lngT = parseFloat(tracto.ubicacion.lng);

                            if (
                                chasisA &&
                                chasisA.ubicacion &&
                                parseFloat(chasisA.ubicacion.lat) !== 0 &&
                                parseFloat(chasisA.ubicacion.lng) !== 0
                            ) {
                                const latC = parseFloat(chasisA.ubicacion.lat);
                                const lngC = parseFloat(chasisA.ubicacion.lng);
                                ...
                                if (dist < metrosAlerta) {
                                    idsToHide.add(chasisA.value);
                                    closeDistancesMap[contId] = (
                                        closeDistancesMap[contId] || []
                                    ).concat(`Chasis A (${Math.round(dist)}m)`);
                                }
                            }
                            ...
                            if (
                                chasisB &&
                                chasisB.ubicacion &&
                                parseFloat(chasisB.ubicacion.lat) !== 0 &&
                                parseFloat(chasisB.ubicacion.lng) !== 0
                            ) {
                                const latC = parseFloat(chasisB.ubicacion.lat);
                                const lngC = parseFloat(chasisB.ubicacion.lng);
                                const dist = getDistanceMeters(
                                    latT,
                                    lngT,
                                    latC,
                                ...
                                if (dist < metrosAlerta) {
                                    idsToHide.add(chasisB.value);
                                    closeDistancesMap[contId] = (
                                        closeDistancesMap[contId] || []
                                    ).concat(`Chasis B (${Math.round(dist)}m)`);
                                }
                            }
                        }
                    });

                    // Filter out close chasis markers from being plotted
                    filteredList = listData.filter(
                        (item) => !idsToHide.has(item.value),
                    );
                }
            }

            // Inject the close distance info directly into the plotted tracto item objects so updating markers InfoWindows can display them
            filteredList.forEach((item) => {
                if (
                    item.TipoEquipo === "Camion" &&
                    closeDistancesMap[item.id_contenendor]
                ) {
                    item.chasisCercanosOcultos =
                        closeDistancesMap[item.id_contenendor].join(", ");
                }
            });

            // Update Panel elements list to display distance alert warnings if active
            $(".dispositivoItem").each(function () {
                const li = $(this);
                const idContenedor = String(li.data("id") ?? "");
                const alertBadge = li.find(".alertaDistanciaBadge");

                if (closeDistancesMap[idContenedor]) {
                    if (alertBadge.length === 0) {
                        li.find(".labelDispositivoPanel").parent().append(`
                            <span class="alertaDistanciaBadge badge bg-warning text-dark mt-1"
                                  style="font-size: 10px; display: inline-block; white-space: normal; text-align: left; max-width: 100%; word-break: break-word; padding: 4px 6px;">
                                ⚠️ Chasis agrupado (Dist. < ${metrosAlerta}m)
                            </span>
                        `);
                    } else {
                        alertBadge.show();
            ...
            // CRITICAL: We pass listData (containing ALL devices, even the filtered out ones) to actualizarUbicacion
            // but inside it we check if it is filtered. Wait! If we pass filteredList to actualizarUbicacion, it renders
            // and saves history for those.
            // Let's pass filteredList to actualizarUbicacion for plotting, but trigger saving database history for ALL devices.
            actualizarUbicacion(filteredList, "Equipo", "all", 0, map, 0, 0);

            // Explicitly remove filtered out markers from the map if they were already plotted previously
            listData.forEach((item) => {
                if (idsToHide.has(item.value)) {
                    const tipoEquipoMarker = normalizarTipoEquipoMapa(
                        item.TipoEquipo ||
                            item.ubicacion?.tipoEquipo ||
                            "Desconocido",
                    );
                    const markerKey = [
                        item.id_contenendor,
            ...
            // Force database logging for the filtered-out devices
            listData.forEach((item) => {
                if (idsToHide.has(item.value)) {
                    const latlocal = parseFloat(item.ubicacion?.lat);
                    const lnglocal = parseFloat(item.ubicacion?.lng);
                    const idConvoyOContenedor = item.id_contenendor;
                    const tipo =
                        (item.tipogps ?? "") + " " + (item.grupo_tipo ?? "");
                    let idProceso = item.id_grupo ?? 0;
                    ...
                    if (idConvoyOContenedor && latlocal && lnglocal) {
                        actualizarUbicacionReal(datasave);
                    }
                }
            });
...
$(document).on("change", "#desactivarAgrupamientoInteligente", function () {
    buscarUbicaciones();
});
