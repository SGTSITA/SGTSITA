        ...
        .then((data) => {
            ...
            contenedoresDisponibles.forEach((cd) => {
                catalogoBusqueda.push({
                    ...
                    id: cd.id_contenedor,
                    value_chasis:
                        cd.contenedor + '|' + cd.imei_chasis + '|' + cd.id_contenedor + '|' + cd.tipoGpsChasis,
...
function obtenerImeisPorConvoyId(convoyId) {
    ...
    let itemFiltradoChasis = detalleConvoys
        ...
        .map(
            (item) =>
                item.num_contenedor + '|' + item.imei_chasis + '|' + item.id_contenedor + '|' + item.tipoGpsChasis,
        );
...
function validarTipo(items) {
    ...
    catalogoBusqueda = catalogoBusqueda.filter(
        (itemFilter) => itemFilter.id !== items.id && itemFilter.value !== items.value,
    );

    const listaPanel = document.getElementById('ElementosRastreoPanel');

    elementoPanelRastro.forEach((item) => {
        const existe = Array.from(listaPanel.children).some((li) => li.dataset.valor === item.id + '|' + item.tipo);
        ...
        if (!existe) {
            ...
            content.classList.add('d-flex', 'justify-content-between', 'align-items-center', 'p-2');
                ...
                dataUbi.forEach((item, index) => {
                    ...
                    if (continueShowing) {
                        ...
                        if (markers[KEYITEM + '|' + item.contenedor + '|' + item.ubicacion.tipoEquipo]) {
                            markers[KEYITEM + '|' + item.contenedor + '|' + item.ubicacion.tipoEquipo].setPosition({
                                lat: latlocal,
                                lng: lnglocal,
                            });
                        } else {
                ...
                `;
                                    ...
                                    google.maps.event.addListenerOnce(infoWindow, 'domready', () => {
                                        const btn = document.getElementById(
                                            `btnRuta_${KEYITEM + '|' + item.contenedor + '|' + item.ubicacion.tipoEquipo}`,
                                        );
                                        const infoSpan = document.getElementById(
                                            `infoRuta_${KEYITEM + '|' + item.contenedor + '|' + item.ubicacion.tipoEquipo}`,
                                        );
