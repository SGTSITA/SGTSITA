...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var gridOptions = {
        ...
        columnDefs: [
            ...
            {
                headerName: "ContenedorPrincipal",
                field: "labelContenedor",
                hide: true,
            },
            { headerName: "Full", field: "referencia_full", hide: true },
            {
                headerName: "Sub Cliente",
                field: "subcliente",
                width: 80,
                hide: true,
            },
...

            {
                ...
                cellRenderer: function (params) {
                    ...
                    else if (params.data.estatus === "Cancelada")
                        color = "danger";
                    else if (params.data.estatus === "Pendiente")
                        color = "warning";
            ...
            { header: "", field: "valores", hide: true },
        ],
        ...
        if (gridDiv) {
            ...
            if (algunaSinValores) {
                Swal.fire(
                    ...
                    "Uno o más viajes seleccionados no tienen valores registrados. Puedes continuar con la planeación y capturarlos al finalizar.",
                    "info",
                );
            }
            ...
            let numContenedorLabel = document.querySelectorAll(
                ".numContenedorLabel",
            );
            let nombreClienteLabel = document.querySelectorAll(
                ".nombreClienteLabel",
            );
            ...
            let direccionEntregaSpan =
                document.querySelector("#direccionEntrega");
            //viaje subcontratado
            let pesoContenedorSubSpan =
                document.querySelector("#pesoContenedorSub");
            let direccionEntregaSubSpan = document.querySelector(
                "#direccionEntregaSub",
            );
            ...
            seleccion.forEach((contenedor) => {
                ...
                isFull =
                    !isFull && contenedor.referencia_full?.length > 0
                        ? true
                        : isFull;
                localStorage.setItem(
                    "numContenedor",
                    JSON.stringify(contenedores),
                );
                ...
                if (direccionEntregaSpan) {
                    direccionEntregaSpan.textContent =
                        contenedor.direccion_entrega;
                }
                if (precioViajeSpan) {
                ...
                if (pesoContenedorSubSpan) {
                    pesoContenedorSubSpan.textContent =
                        contenedor.peso_contenedor;
                }
                if (direccionEntregaSubSpan) {
                    direccionEntregaSubSpan.textContent =
                        contenedor.direccion_entrega;
                }
            });
            ...
            cmbTipoUnidad.value =
                seleccion.length == 2 || isFull ? "Full" : "Sencillo";
            cmbTipoUnidad.dispatchEvent(new Event("change"));

            numContenedorLabel.forEach(
                (lb) => (lb.textContent = contenedoresLabel),
            );
            bntNextOne.disabled = false;

            nombreClienteLabel.forEach(
                (cl) => (cl.textContent = cliente_Sublcientelabel),
            );
        }
    }
});
...
function calculateTotal() {
    var precio =
        parseFloat(reverseMoneyFormat($("#precio_proveedor").val())) || 0;
    var burreo =
        parseFloat(reverseMoneyFormat($("#burreo_proveedor").val())) || 0;
    var maniobra =
        parseFloat(reverseMoneyFormat($("#maniobra_proveedor").val())) || 0;
    var estadia =
        parseFloat(reverseMoneyFormat($("#estadia_proveedor").val())) || 0;
    var otro = parseFloat(reverseMoneyFormat($("#otro_proveedor").val())) || 0;

    var sobrepeso =
        parseFloat(reverseMoneyFormat($("#sobrepeso_proveedor").val())) || 0;
    var cantidadsob =
        parseFloat(
            reverseMoneyFormat($("#cantidad_sobrepeso_proveedor").val()),
        ) || 0;
    ...
    const baseFactura =
        parseFloat(
            reverseMoneyFormat(document.getElementById("base_factura").value),
        ) || 0;
    var iva = baseFactura * tasa_iva;
    var retencion = baseFactura * tasa_retencion;

    document.getElementById("iva_proveedor").value = moneyFormat(
        iva.toFixed(2),
    );
    document.getElementById("retencion_proveedor").value = moneyFormat(
        retencion.toFixed(2),
    );
    var total =
        precio + burreo + maniobra + estadia + otro + iva + sobre - retencion;

    const baseTaref = total - baseFactura - iva + retencion;

    document.getElementById("base_taref").value = moneyFormat(
        baseTaref.toFixed(2),
    );
...
async function programarViaje() {
    ...
    if (tipoViaje == "propio") {
        ...
        for (const fila of filas) {
            ...
            const pagoInmediato = fila.querySelector(
                '[name="gasto_pago_inmediato[]"]',
            ).checked;
            const banco =
                fila.querySelector('[name="gasto_banco_id[]"]').value || null;
            ...
            if (pagoInmediato && !banco) {
                Swal.fire(
                    "Banco inválido",
                    "Debe seleccionar un banco para el gasto '" +
                        motivo +
                        "' que se pagará de inmediato.",
                    "warning",
                );
                hayErrorGastos = true;
                break;
            ...
            if (pagoInmediato && !fechaAplicacion) {
                Swal.fire(
                    "Fecha inválido",
                    "Debe seleccionar fecha aplicacion para el gasto '" +
                        motivo +
                        "' que se pagará de inmediato.",
                    "warning",
                );
