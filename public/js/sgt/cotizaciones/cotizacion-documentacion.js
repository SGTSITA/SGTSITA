...
const ragCellClassRules = {
    "badge badge-light-info fs-base": (params) =>
        params.value === "Viaje solicitado",
    "badge badge-light-warning fs-base": (params) =>
        params.value === "Documentos Faltantes",
    "badge badge-light-success fs-base": (params) =>
        params.value === "Aprobada",
    "badge badge-light-primary fs-base": (params) =>
        params.value === "Planeado",
    "badge badge-light-danger fs-base": (params) =>
        params.value === "Cancelada",
};
...
const gridOptions = {
    ...
    onSelectionChanged: (event) => {
        ...
        const hayLocal = rows.some((r) => r.convertido_local);

        if (hayLocal) {
            motivoBloqueo = "No puedes convertir registros que ya son locales";
            bloquearBoton(btn);
        } else {
            motivoBloqueo = "";
            habilitarBoton(btn);
    ...
    columnDefs: [
        ...
        { field: "convertido_local", hide: true },
        {
...
function RegresarForaneoLocal() {
    let seleccion = apiGrid.getSelectedRows();
    let btnforaneo = document.getElementById("btnRegresarLocal");
    ...
    Swal.fire({
        title: "¿Estás seguro?",
        text: "Esta accion convertirá los contenedores seleccionados a viaje local.",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Sí, continuar",
        cancelButtonText: "No, cancelar",
        ...
        if (result.isConfirmed) {
            let _token = document
                ...
                .getAttribute("content");
            $.ajax({
                url: "/cotizaciones/transformar/revertirforaneo",
                type: "post",
                data: { _token, seleccion },
                beforeSend: () => {},
                success: (response) => {
