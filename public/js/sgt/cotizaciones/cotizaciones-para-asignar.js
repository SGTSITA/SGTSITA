...
function asignarContenedores() {
    ...
    if ($("#cmbEmpresa").val() == "") {
        Swal.fire(
            "Seleccione Empresa",
            "Aún no ha seleccionado Empresa, este es un campo requerido",
            "warning",
        );
        return false;
    }
    ...
    if (contenedores.length == 0) {
        Swal.fire(
            ...
            "Para realizar la asignación debe seleccionar al menos un contenedor",
            "warning",
        );
        return false;
    }
...
function confirmarAsignarContenedores(contenedores) {
    ...
    let proveedor = $("#id_transportista").val();
    let seleccionContenedores = JSON.stringify(contenedores);
    $.ajax({
        url: "/cotizaciones/asignar/empresa",
        type: "post",
        data: { _token, seleccionContenedores, empresa, proveedor },
        beforeSend: () => {},
        success: (response) => {
...
let cmbEmpresa = document.querySelector("#cmbEmpresa");
...
function getTranspotistas() {
    ...
    let proveedor = cmbEmpresa.value;
    $.ajax({
        type: "post",
        ...
        success: (response) => {
            ...
            selectTransport.add(new Option("Ninguno", ""));
