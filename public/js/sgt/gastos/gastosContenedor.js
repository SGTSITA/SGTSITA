...
function applyPaymentGastos() {
        ...
        if (node.isSelected()) {
            ...
            gastosPagar.push({
                ...
                fecha_inicio: row.fecha_inicio
                    ? row.fecha_inicio.format("YYYY-MM-DD HH:mm:ss")
                    : null,
                fecha_fin: row.fecha_fin
                    ? row.fecha_fin.format("YYYY-MM-DD HH:mm:ss")
                    : null,
                FechaGasto: row.FechaGasto
                    ? row.FechaGasto.format("YYYY-MM-DD")
                    : null,
            });
    ...
    $.ajax({
        url: "/gastos/payGxp",
        type: "post",
        data: { totalPago, bank, gastosPagar, fecha_aplicacion, _token },
        ...
        error: () => {
            ...
            Swal.fire(
                "Error inesperado",
                "Ocurrió un error mientras procesamos su solicitud",
                "error",
            );
        },
    });
    ...
    $.ajax({
        ...
        success: (response) => {
            gridDataOriginal = (response.contenedores || []).map((row) => ({
                ...row,
                fecha_inicio: row.fecha_inicio
                    ? moment(row.fecha_inicio, "YYYY-MM-DD HH:mm:ss")
                    : moment(row.fecha, "YYYY-MM-DD"),
                fecha_fin: row.fecha_fin
                    ? moment(row.fecha_fin, "YYYY-MM-DD HH:mm:ss")
                    : moment(row.fecha, "YYYY-MM-DD"),
                FechaGasto: row.FechaGasto
                    ? moment(row.FechaGasto, "YYYY-MM-DD")
                    : moment(row.fecha, "YYYY-MM-DD"),
            }));
