...
async function getBancos() {
    ...
    data.forEach((banco) => {
        dataBancos.push(
            formatoConsecutivo(banco.id) +
                " - " +
                banco.nombre_beneficiario +
                " - " +
                banco.nombre_banco,
        );
    });
...
const gridOptions = {
    ...
    columnDefs: [
        ...
        { field: "Categoria", width: 150, filter: true, floatingFilter: true },
        ...
        { field: "Diferido", filter: true, floatingFilter: true },
...
function handleSelection(input) {
    ...
    if (input.value === "Equipo") {
        document
            .querySelector("#aplicacion-equipoGeneral")
