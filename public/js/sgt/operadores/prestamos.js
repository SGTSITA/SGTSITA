...
data.historial.forEach(mov => {
    const fecha = mov.fecha_pago ? new Date(mov.fecha_pago).toLocaleDateString() : new Date(mov.created_at).toLocaleDateString();
    ...
    if (mov.tipo_origen && (mov.tipo_origen.toLowerCase() === 'directo' || mov.tipo_origen.toLowerCase() === 'liquidacion')) {
        deuda -= monto;
    } else {
        deuda += monto;
    }
