...
class MissionResultRenderer {
  ...
  const gridOptions = {
   ...
   columnDefs: [
     ...
     { field: "ViajesRealizados",width: 150 },
 ...
 // const gridInstance = new agGrid.Grid(myGridElement, gridOptions);
  ...
  let btnVerDetalle = document.querySelector('#openPay')
   
   function getViajesOperadores(){
    var _token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
    $.ajax({
        url: '/liquidaciones/viajes/pagos-operadores',
        type: 'post',
        data: {_token},
        beforeSend:()=>{},
        success:(response)=>{
   ...
   function verDetallePagos(){

    let operador = apiGrid.getSelectedRows();
    if(operador.length <= 0){
        Swal.fire('Seleccione un operador','Debe seleccionar un operador de la lista','warning');
        return;
    } 

    const link = document.createElement('a');
    link.href = `/liquidaciones/show/${operador[0].IdOperador}`;
