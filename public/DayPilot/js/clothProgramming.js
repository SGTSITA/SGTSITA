 ...
 **/
        ...
        function GuardarProgTejido(){
            ...
            $.ajax({
                ...
                type:'post',
                data:{ _token:_token, events : JSON.stringify(events), requiredQty: requiredQty,weavingProgramItems: weavingProgramItems, article: article, article_id: article_id,demandItems:demandItems,threadComposition:threadComposition},
                beforeSend:()=>{
                    Swal.fire({
        ...
        function detalleDiario(tarifa,diasProd,startDate,numMachine){
            ...
            $.ajax({
                ...
                type:'get',
                data: {tarifa: tarifa,num_days_production: diasProd,startDate: startDate, requiredQty: requiredQty,numMachine:numMachine},
                beforeSend:()=>{
            ...
            $("#tabclothProgram>tbody>tr").detach();
                    if(programByMachine != null){
                        programByMachine.forEach((s)=>{
                          $("#tabclothProgram>tbody").append("<tr><td>"+s.maquina+"</td><td>"+s.Qty+"Kg</td><td>"+s.startDate+"</td><td>"+s.endDate+"</td><td>"+Math.ceil(s.daysForProduce)+"</td></tr>");
                       });
                    }
        ...
        async function getMachines(){
            ...
            const result = await $.ajax({
                ...
                data:{startDate: dp.startDate,methodProgram:methodProgram,article:article,requiredQty:requiredQty, threadComposition: threadComposition},
                beforeSend:()=>{
                    Swal.fire({
                ...
                success:(resp)=>{
                    ...
                    if(mySugesstions != null){
                        mySugesstions.forEach((s)=>{
                          $("#tabSuggestions>tbody").append("<tr><td>"+s.maquina+"</td><td>"+s.capacidad+"Kg</td><td>"+s.startDate+"</td><td>"+s.endDate+"</td><td>"+Math.ceil(s.daysForProduce)+"</td><td><a href='#' onclick='detalleDiario("+s.capacidad+","+Math.ceil(s.daysForProduce)+","+'"'+s.startDate+'"'+","+s.maquina+")'>Detalles</a></td></tr>");
                       });
                    }
       ...
       /* dp.onTimeRangeSelected = function (args) {
        ...
        dp.onEventMove = function (args) {
            ...
            if (args.ctrl){
                var forComplete = $("#qtyForComplete").data('forcomplete');
                if(forComplete <= 0){
                    Swal.fire('Producción multiple no permitida','No es posible agregar producción a una nueva maquina porque la cantidad requerida está completa','warning');
                    return false;
                }
