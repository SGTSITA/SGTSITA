 ...
 */
 ...
 function GuardarProgTejido(){
     ...
     $.ajax({
         ...
         type:'post',
         data:{ _token:_token, events : JSON.stringify(events), requiredQty: requiredQty, article: article, article_id: article_id,demandItems:demandItems},
         beforeSend:()=>{
            Swal.fire({
         ...
         error:(err)=>{
            Swal.fire('Error','No hemos podido procesar su solicitud, es posible que no se hayan registrado los cambios ','error');
         }
     })
 }
 ...
 function getTarifas(clothList){
    $.ajax({
        ...
        error:(e)=>{
            Swal.close();
            Swal.fire('Error: '+e.statusText,'Ha ocurrido un error, no pudimos procesar su solicitud. Es posible que el modulo no funcione correctamete','error');
        }
    })
 }
