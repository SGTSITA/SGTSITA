...
var Fileuploader = function (c, f, g, h) {
    return (
        (this.req = g),
        (this.res = h),
        (this.options = extendDefaults.call(this, f, {
            limit: null,
            maxSize: null,
            fileMaxSize: null,
            extensions: null,
...
((Fileuploader.prototype.getFileList = function (c) {
    ...
    (Fileuploader.prototype.generateInput = function () {
        ...
        for (var g in f) {
            ...
            h &&
                ('limit' == g || 'maxSize' == g || 'fileMaxSize' == g
                    ? c.push({
                          key: j,
                          value: h,
                      })
    ...
    (Fileuploader.prototype.upload = function (c) {
                ...
                _setStatus: function (j, l, m) {
                    ...
                    if (
                        (null !== j &&
                            (!0 === j
                                ? (n.isSuccess = !0)
                                : ((n.isSuccess = !1), (n.hasWarnings = !0), l && n.warnings.push(l))),
                        m && 'function' == typeof o)
                    ) {
                        var p = !1;
                        if (
                            (f.options.files.forEach(function (q) {
                                isset(q._processing) &&
                                    ((q._processingCallback = function () {
                                        (delete q._processing,
                                            delete q._processingCallback,
                                            n._setStatus(null, null, !0));
                                    }),
                                    (p = !0));
                            }),
                            p)
        ...
        f.multer[g](f.field.name)(f.req, f.res, function (j) {
            ...
            ((f.field.input = l.length),
                (f.field.listInput = getListInputFiles.call(f, f.field.name)),
                !0 === o
                    ? handleUpload.call(f, l, h)
                    : h._setStatus(
                          !1,
                          {
                              code: o,
...
function handleUpload(c, f) {
    for (var g = this, h = g.options, j = g.field.listInput, l = 0; l < c.length; l++) {
        ...
        ((m.oldname = m.name),
            (m.oldtitle = p.title),
            (m.extension = p.extension),
            (m.title = m.oldtitle),
            (m.size2 = formatSize(m.size)),
            (m.name = generateFilename(h, m)),
            (m.title = pathinfo(m.name).title),
            (m.file = h.uploadDir + m.name),
            (m.replaced = fs.existsSync(g.options.uploadDir + m.name)),
            (m.date = q));
        ...
        if (!0 !== r) {
            ...
            var s = 0;
            (j && ((s = j.list.indexOf(n)), (m.listProps = j.values[s]), j.list.splice(s, 1), j.values.splice(s, 1)),
                f.files.push(m));
        }
    }
    return (
        f.hasWarnings ||
            f.files.forEach(function (t, u) {
                h.move_uploaded_file(t)
                    ? ((t.uploaded = !0), delete t.chunked, delete t.tmp, delete t.tmp_name, h.files.push(t))
                    : f.files.splice(u, 1);
            }),
        j &&
            h.files.forEach(function (t) {
...
function validate_files(c) {
    if (c instanceof Array) {
        ...
        if (
            ...
            (c.reduce((g, h) => g + h.size, 0) + this.options.files.reduce((g, h) => g + h.size, 0)) / 1e6 >
                this.options.maxSize
        )
            return 'MAX_SIZE';
    } else {
        if (
            ...
            -1 == this.options.extensions.indexOf(c.extension) &&
            -1 == this.options.extensions.indexOf(c.type)
        )
            return 'INVALID_TYPE';
...
function parseVariables(c, f) {
    return (
        (c += ''),
        (c = c.replace(/\{file_name\}/g, f.name)),
        (c = c.replace(/\{file_size\}/g, f.size)),
        (c = c.replace(/\{extension\}/g, f.extension)),
        c
    );
}

function random_string(c) {
    for (var f = '_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', g = '', h = 0; h < c; h++)
