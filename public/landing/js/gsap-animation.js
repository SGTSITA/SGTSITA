(function ($) {
    ...
    var pbmit_active_hover = function () {
        var pbmit_var = jQuery(
            '.pbmit-element-static-box-style-1,.pbmit-element-static-box-style-2,.pbmit-element-service-style-3',
        );
        ...
        pbmit_var.each(function () {
            var pbmit_Class =
                '.pbmit-static-box-style-1,.pbmit-hover-inner li,.pbmit-service-style-3:not(.swiper-slide-duplicate)';
    ...
    var pbmit_burger_menu = function () {
        if (jQuery('.header-style-3').length > 0) {
            jQuery('.header-style-3 .pbmit-header-overlay .pbmit-logo-area')
                .clone()
                .appendTo('.pbmit-burger-headerarea');
            jQuery('.header-style-3 .pbmit-header-overlay .pbmit-search-cart-box')
                .clone()
                .appendTo('.pbmit-burger-headerarea');

            jQuery('.header-style-3 .pbmit-header-overlay .main-navigation')
                .clone()
                .appendTo('.pbmit-burger-menu-area-inner')
                .insertBefore('.pbmit-burger-content');
            jQuery(
                '.pbmit-burger-menu-area .main-navigation, .pbmit-burger-menu-area .main-navigation ul, .pbmit-burger-menu-area .main-navigation ul li, .pbmit-burger-menu-area .main-navigation ul li a',
            ).removeAttr('id');
            ...
            jQuery('.pbmit-burger-menu-area ul.menu li:has(ul) > a').after(
                "<span class='sub-menu-toggle'><i class='pbmit-base-icon-down-open-big'></i></span>",
            );
            ...
            jQuery('.pbmit-burger-menu-area .sub-menu-toggle').on('click', function () {
                if (jQuery(this).siblings('.sub-menu, .children').css('display') == 'block') {
                    ...
                    jQuery('i', jQuery(this))
                        ...
                        .addClass('pbmit-base-icon-down-open-big');
                } else {
                    jQuery(this).siblings('.sub-menu, .children').slideDown();
                    jQuery('i', jQuery(this))
                        .removeClass('pbmit-base-icon-down-open-big')
