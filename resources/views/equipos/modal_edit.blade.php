<div class="modal fade" id="equipoEditModal-{{ $item->id }}" tabindex="-1" aria-labelledby="equipoModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0 shadow-sm">
            ...
            <form method="POST" action="{{ route('update.equipos', $item->id) }}" enctype="multipart/form-data"
                class="form-editar-equipo" data-id="{{ $item->id }}" data-tipo="{{ $item->tipo }}">
                @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            @if ($item->tarjeta_circulacion)
                                <small class="text-muted mt-1 d-block">
                                    Documento actual: <strong>{{ $item->tarjeta_circulacion }}</strong>
                                </small>
                            @endif
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            @if ($item->poliza_seguro)
                                <small class="text-muted mt-1 d-block">
                                    Documento actual: <strong>{{ $item->poliza_seguro }}</strong>
                                </small>
                            @endif
                        </div>
                        ...
                        @if ($item->tipo === 'Chasis / Plataforma')
                            <div class="col-md-12">
                                ...
                                <select name="folio" class="form-select">
                                    ...
                                    <option value="B9 40P" {{ $item->folio === 'B9 40P' ? 'selected' : '' }}>B9 40P
                                    </option>
                                    <option value="B10 20P" {{ $item->folio === 'B10 20P' ? 'selected' : '' }}>B10 20P
                                    </option>
                                    <option value="B11 20/40P" {{ $item->folio === 'B11 20/40P' ? 'selected' : '' }}>
                                        B11 20/40P</option>
                                    <option value="B12 Abatible"
                                        {{ $item->folio === 'B12 Abatible' ? 'selected' : '' }}>B12 Abatible</option>
                                    <option value="B13 Retractil"
                                        {{ $item->folio === 'B13 Retractil' ? 'selected' : '' }}>B13 Retractil</option>
                                </select>
                            </div>
                        @endif
                    </div>
