                ...
                <div class="card">
                    <div class="card-header pb-0">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <h3 id="card_title">Historial de Pagos (CxP)</h3>
                            <div class="d-flex gap-2 align-items-center">
                                <a href="{{ route('cobros_pagos.exportar_excel', array_merge(request()->all(), ['tipo' => 'cxp'])) }}" class="btn btn-sm btn-outline-success mb-0">
                                    <i class="fas fa-file-excel"></i> Excel
                                </a>
                                <a href="{{ route('cobros_pagos.exportar_pdf', array_merge(request()->all(), ['tipo' => 'cxp'])) }}" class="btn btn-sm btn-outline-danger mb-0">
                                    <i class="fas fa-file-pdf"></i> PDF
                                </a>
                                @if(request('proveedor_id'))
                                    <a class="btn btn-sm btn-secondary mb-0" href="{{ route('show.pagar', request('proveedor_id')) }}">
                                        <img src="{{ asset('img/icon/izquierda_white.png') }}" alt="" width="18px"> Regresar
                                    </a>
                                @else
                                    <a class="btn btn-sm btn-secondary mb-0" href="{{ route('index.pagar') }}">
                                        <img src="{{ asset('img/icon/izquierda_white.png') }}" alt="" width="18px"> Regresar
                                    </a>
                                @endif
                            </div>
                    ...
                    <div class="card-body pt-2">
                        <!-- Filtros -->
                        <form method="GET" action="{{ route('cobros_pagos.historico_pagar') }}" class="mb-4">
                            <div class="row align-items-end">
                                <div class="col-md-3 mb-2">
                                    <label for="proveedor_id" class="form-label text-sm mb-1">Proveedor</label>
                                    <select name="proveedor_id" id="proveedor_id" class="form-select form-select-sm select2-filter" style="width: 100%;">
                                ...
                                <div class="col-md-3 mb-2">
                                    <label for="cliente_id" class="form-label text-sm mb-1">Cliente (Cotizaciones)</label>
                                    <select name="cliente_id" id="cliente_id" class="form-select form-select-sm select2-filter" style="width: 100%;">
                                ...
                                <div class="col-md-2 mb-2">
                                    ...
                                    <input type="text" name="num_contenedor" id="num_contenedor" class="form-control form-control-sm" value="{{ request('num_contenedor') }}" placeholder="Ej. MAEU123456">
                                </div>
                                ...
                                <div class="col-md-1 mb-2 d-flex gap-2">
                                    ...
                                    <a href="{{ route('cobros_pagos.historico_pagar') }}" class="btn btn-sm btn-outline-secondary mb-0 p-2 btn-reset-filter" title="Quitar filtros" data-bs-toggle="tooltip" data-bs-placement="top">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-sm align-middle text-sm" style="font-size: 0.85rem;">
                                ...
                                <tbody>
                                    @forelse($pagos as $pago)
                                        @php
                                            $totalPagado = $pago->monto_A + $pago->monto_B;
                                        @endphp
                                        <tr class="main-row" data-target="#collapsePago{{ $pago->id }}" style="cursor: pointer;">
                                            ...
                                            <td class="text-truncate" style="max-width: 200px;" title="{{ $pago->observaciones }}">{{ $pago->observaciones ?? '-' }}</td>
                                            <td class="text-center action-cell d-flex justify-content-center align-items-center gap-1">
                                                 <a href="{{ route('cobros_pagos.comprobante_pdf', $pago->id) }}" class="btn btn-xs btn-outline-info py-1 px-2 m-0" title="Descargar Comprobante PDF">
                                                     PDF
                                                 </a>
                                                 <button type="button" class="btn btn-xs btn-danger text-white py-1 px-2 m-0 btn-eliminar-pago" data-url="{{ route('cobros_pagos.eliminar', $pago->id) }}" data-fecha="{{ optional($pago->fechaAplicacion1)->format('Y-m-d') ?? date('Y-m-d') }}">
                                                     Eliminar
                                                 </button>
                                             </td>
                                        </tr>
                                        ...
                                        <tr class="collapse" id="collapsePago{{ $pago->id }}">
                                            <td colspan="10" class="bg-light p-3">
                                                <div class="card card-body shadow-none border m-0">
                                                    <h6 class="mb-2">Cotizaciones/Viajes vinculados al Pago #{{ $pago->id }}</h6>
                                                    <div class="table-responsive">
                                                        <table class="table table-bordered table-sm m-0 text-xs" style="font-size: 0.8rem;">
                                                            <thead>
                                                                <tr class="table-secondary">
                                                                    ...
                                                                    <th>Saldo Restante Proveedor</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                @forelse($pago->detalles as $detalle)
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>${{ number_format($detalle->cotizacion->prove_restante ?? 0, 2) }}</td>
                                                                    </tr>
                                                                @empty
                                                                    <tr>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="10" class="text-center">No se encontraron registros de pagos.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            const initStart = $('#fecha_inicio').val() ? moment($('#fecha_inicio').val()) : moment().subtract(6, "days").startOf("day");
            ...
            $('.btn-eliminar-pago').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    preConfirm: () => {
                        const dateVal = document.getElementById('fecha_cancelacion_input').value;
                        if (!dateVal) {
                            Swal.showValidationMessage('¡Debe seleccionar una fecha de cancelación!');
                        }
                        return dateVal;
                    }
                }).then((result) => {
